# ADR-001: Keep transport-raw as an Independent Gnalloy Module
...
## Decision
`transport-raw` is published as `gnalloy.org/transport-raw` and owns this responsibility: Raw IP protocol transport for Gnalloy with packet messages and explicit privileged runtime boundaries.
